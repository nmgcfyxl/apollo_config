...
class ApolloListener(ApolloClient):
    ...
    def __init__(self, app_id, cluster='default', config_path=None, timeout=35, ip=None):
        ...
        if os.path.exists(config_path):
            ...
            raise ValueError("config_path:{} is not file path, exists and dict".format(config_path))

        config_server_url = config.get(env.upper(), config.get(env.lower()))
        if not config_server_url:
            raise ValueError("{} undefined".format(env))

        super(ApolloListener, self).__init__(app_id, cluster=cluster, config_server_url=config_server_url,
                                             timeout=timeout, ip=ip)

    def _uncached_http_get(self, namespace='application'):
        super(ApolloListener, self)._uncached_http_get(namespace)
        config = self._cache[namespace]
